...
public class OTree {
						   ...
						   Collection<SlotItem> pUnassignedList )
   {
	...
	/**
	 * Runs the initial branches through a thread pool of MAX_THREADS. Each Thread runs with a timeout in place and this will return the first
	 * solution generated by any of the threads, or null if all the threads timeout.
	 * 
	 * @return An OTree with a valid solution or null if no solution could be found in the specified time.
	 * @throws InterruptedException 
	 */
	public OTree genSolutionAsync( )
	{
		// Local Variables
		ExecutorService pExecService;
		Set<CallableBranch> pCallables = new HashSet< CallableBranch >();
		OTree pReturnSolution = null;		
	
		pCallables.clear();
		pExecService = Executors.newFixedThreadPool(MAX_THREADS);
	...
	private void altern()
	{
		...
		if( m_pLeafs.isEmpty() && !m_pUnassignedList.isEmpty() )
		{
			...
			for( Evaluated pEval : pLeafs )
			{
				...
				pNewTree = new OTree( m_pDept, pNxtAssign, m_pUnassignedList );
				m_pLeafs.add( pNewTree );
			}
		}
	}
